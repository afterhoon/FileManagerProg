...
public class FileManager extends JFrame {
	...
	void makeGUI() {
		...
		/* 파일 컨트롤 버튼 */
		String controlStr[] = {"images\\upload.png", "images\\download.png", "images\\DELETE.png"};
		...
		JLabel sortTitle = new JLabel("정렬");
	...
	void initDB(){
		...
		for(File tempFile : fileList) {
			if(tempFile.isFile()) {
				...
				switch(selfo.getType()){
				case 'i': vImage.addElement(selfo); break;
				case 'd': vDocum.addElement(selfo); break;
				case 'a': vAudio.addElement(selfo); break;
				case 'x': vEtc.addElement(selfo); break;
				}
   ...
   void initFile() {
	...
	void searchFile(String sch) {
		...
		if(vSearch.size()==0){
			JOptionPane.showMessageDialog(null, "존재하지 않는 파일입니다.","Message",JOptionPane.ERROR_MESSAGE);

			for(int i = 0 ; i < vFile.size() ; i++)   vSearch.addElement(vFile.elementAt(i));
	...
	class UploadActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			selInfo.setAddr(databaseRoot + selInfo.getFileName());
			vFile.addElement(selInfo); //경로변경
	...
	void deleteDB(FileInfo in){
		...
		for(File tempFile : fileList) {
			...
			if(tempFile.getName().equals(in.getFileName()))   tempFile.delete();
		}

	}

	//강지우
	void uploadDB(FileInfo in) {   
		//db에 파일 복사, 파일 경로 변경 
	...
	class MediaPanel extends JPanel {
		...
		public void setMediaPanel(int num) {
			...
			case 'i':
				...
				break;
			case 'd': 
				if(vCurr.elementAt(num + (page-1)*viewWid*viewHei).getFileName().contains("hwp")){
					imageIcon=new ImageIcon("images\\hwp.png");
					mBtn.setImage(imageIcon, vCurr.elementAt(num + (page-1)*viewWid*viewHei));
				}
				else mBtn.setDocum(vCurr.elementAt(num + (page-1)*viewWid*viewHei));
				break;
			case 'a':
				imageIcon = new ImageIcon("images\\audio2.png");
				mBtn.setImage(imageIcon, vCurr.elementAt(num + (page-1)*viewWid*viewHei));
				break;
			default:
				imageIcon=new ImageIcon("images\\etc.png");
				mBtn.setImage(imageIcon, vCurr.elementAt(num + (page-1)*viewWid*viewHei));
				break;
			}
					...
					if(b.getFileInfo() == targetFile) {
						...
						case 'd':
							try {
								//프로그램명 매개변수  넣어주면 실행
								//파일 경로명 넣어주면 경로명에 있는 파일이 노트패드로 실행
								System.out.println("확인: " + info.getExt() + " : " + "hwp" + " ? " + (info.getExt().equals("hwp")?"TRUE":"FALSE"));
								r1.exec((info.getExt().equals("hwp")? hwpViewer : documentViewer) + " " + targetFile.getAddr()); 
